...
{
      File dataFile = SD.open("SkidPad.txt", FILE_WRITE);
      ...
      if (dataFile) {
        ...
        Serial.println("Error opening SD for " + String("Skidpad.txt"));
      }
}
...
void loop() {
 ...
 //Serial.println("To loop"); 
  if(!Ready){
    dataString += (String("|Accel(M/s^2)XYZ | Gyro(rad/s)XYZ | ShockPots - FrontRight, FrontLeft, RearRight, RearLeft \n"));
    delay(100);
    Ready = true;
  }
